...
Use this skill when the user wants Rust (kagent/biofoundry_llm/biofoundry_os) to stay logically identical to Python (biofoundry_cli/biofoundry_llm/biofoundry_os), excluding UI and login/auth. This includes code and tests: Rust behavior and tests must be fully synchronized with Python changes.

Note: The Rust binary is named `kagent`. User-facing CLI/output text in Rust must use `kagent`
instead of `biofoundry` to match the Rust command name.
...
Common mappings:
- `src/biofoundry_cli/llm.py` -> `rust/kagent/src/llm.rs`
- `src/biofoundry_cli/soul/*` -> `rust/kagent/src/soul/*`
- `src/biofoundry_cli/tools/*` -> `rust/kagent/src/tools/*`
- `src/biofoundry_cli/utils/*` -> `rust/kagent/src/utils/*`
- `src/biofoundry_cli/wire/*` -> `rust/kagent/src/wire/*`
- `packages/biofoundry_llm/*` -> `rust/biofoundry_llm/*`
- `packages/biofoundry_os/*` -> `rust/biofoundry_os/*`
...
- Ensure E2E parity: use the existing Python E2E suite against the Rust binary with the
  configured wire-command override env var (do not rewrite E2E in Rust). All E2E cases must
  pass or the gap must be documented.
...
- Run E2E tests with the wire command swapped to Rust, and ensure they pass.
...
- [ ] Rust E2E passed with the wire command override
