...
biofoundry synpan-mcp   # unified SynPan MCP server
```
...
- `src/biofoundry_cli/tools/`: built-in shell, file, web, plan, multiagent, and sanger tools.
...
- `src/biofoundry_cli/synpan/`: unified SynPan CIAI device driver and third-party craft-platform adapter.
- `Knowledge/`: bundled domain knowledge and packaged Biofoundry skills.

## SynPan MCP Adapter

`biofoundry synpan-mcp` starts a unified stdio MCP server that exposes two groups of tools:

- **CIAI device endpoints**: `synpan_get_info`, `synpan_run_function`, `synpan_set_parameters`, etc.
- **Third-party craft platform**: `getWorkCellList`, `getCraftList`, `createOrder`, `operateOrder`, etc.

Environment variables:

- CIAI driver: `SYNPAN_BASE_URL`, `SYNPAN_TOKEN`, `SYNPAN_TIMEOUT_SECONDS`, etc.
- Third-party platform: `SYNPAN_PLATFORM_BASE_URL`, `SYNPAN_PLATFORM_TOKEN`, `SYNPAN_PLATFORM_TIMEOUT_MS`; legacy `XINGPAN_*` variables are still accepted as fallbacks.

See [`docs/synpan-mcp.md`](./docs/synpan-mcp.md) for details.
...
- This README is the English version; the default Chinese version is at [`README.md`](./README.md).
