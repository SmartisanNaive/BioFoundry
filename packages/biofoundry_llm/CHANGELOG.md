...
- Implement `RetryableChatProvider` in multiple OpenAI-compatible providers
...
- Remove provider-specific default temperature setting based on model name
...
- Add a provider file-upload interface that returns `VideoURLPart`
- Add video upload support for provider-backed file APIs
...
- Add `convert_mcp_content` util to convert MCP content type to biofoundry_llm content type
...
- Make the `contrib` module an optional extra that can be installed with `uv add "biofoundry_llm[contrib]"`
...
- Fix an OpenAI-compatible chat provider panicking when `stream` is `False`
...
- Fix interleaved thinking for OpenAI-compatible chat providers
...
- Catch httpx exceptions correctly in OpenAI-compatible providers
...
- Set default temperature settings for specific model families based on model name
