try:
    ...
    raise ModuleNotFoundError(
        ...
        'Install with `pip install "biofoundry_llm[contrib]"`.'
    ) from exc
...
from biofoundry_llm.message import (
    AudioURLPart,
    ContentPart,
    ImageURLPart,
    Message,
...
from biofoundry_llm.tooling import Tool as BiofoundryLLMTool
from biofoundry_llm.tooling import ToolReturnValue
...
class GoogleGenAI:
    ...
    async def generate(
        self,
        system_prompt: str,
        tools: Sequence[BiofoundryLLMTool],
...
def tool_to_google_genai(tool: BiofoundryLLMTool) -> Tool:
    """Convert a Biofoundry LLM tool to GoogleGenAI tool format."""
...
def _convert_error(error: Exception) -> ChatProviderError:
    """Convert a GoogleGenAI error to a Biofoundry LLM chat provider error."""
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        from biofoundry_llm.tooling import CallableTool2, ToolOk
        ...
        result = await biofoundry_llm.step(chat, system_prompt, toolset, history)
