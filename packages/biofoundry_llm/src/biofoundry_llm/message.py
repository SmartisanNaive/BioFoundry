...
class ContentPart(BaseModel, ABC, MergeableMixin):
    ...
    For Biofoundry LLM users, you typically do not need to subclass this directly.
    Instead, use the provided subclasses like `TextPart`, `ThinkPart`,
    `ImageURLPart`, etc. Unless you are implementing custom `ChatProvider`s that
    supports new content part types.
    """
...
type Role = Literal[
    ...
    # OpenAI-compatible APIs support system messages in the middle of the conversation.
