...
from biofoundry_llm.message import ContentPart, ToolCall
from biofoundry_llm.utils.jsonschema import deref_json_schema
...
class DisplayBlock(BaseModel, ABC):
    ...
    Unlike `ContentPart`, Biofoundry LLM users may directly subclass `DisplayBlock` to define custom
...
class CallableTool(Tool, ABC):
    ...
    async def call(self, arguments: JsonType) -> ToolReturnValue:
        from biofoundry_llm.tooling.error import ToolValidateError
