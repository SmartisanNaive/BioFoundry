...
if TYPE_CHECKING:
    ...
    def type_check(ssh: SSHFoundryOS) -> None:
        _: FoundryOS = ssh
...
class SSHFoundryOS:
    """
    A Foundry OS implementation that interacts with a remote machine via SSH and SFTP.
    """

    name: str = "ssh"

    class Process:
        """Foundry OS process wrapper around asyncssh.SSHClientProcess."""
        ...
        async def wait(self) -> int:
            ...
            # stdout/stderr remain readable after wait, matching LocalFoundryOS.
    ...
    def normpath(self, path: StrOrFoundryPath) -> FoundryPath:
        return FoundryPath(posixpath.normpath(str(path)))

    def gethome(self) -> FoundryPath:
        return FoundryPath(self._home_dir)

    def getcwd(self) -> FoundryPath:
        return FoundryPath(self._cwd)
    ...
    async def iterdir(self, path: StrOrFoundryPath) -> AsyncGenerator[FoundryPath]:
        biofoundry_os_path = FoundryPath(path) if isinstance(path, str) else path
        for entry in await self._sftp.listdir(str(path)):
            # NOTE: sftp listdir gives . and ..
            if entry in {".", ".."}:
                continue
            yield biofoundry_os_path / entry
    ...
    async def glob(
        ...
        *,
        ...
        for entry in await self._sftp.glob(f"{real_path}/{pattern}"):
            yield FoundryPath(await self._sftp.realpath(str(entry)))
    ...
    async def exec(self, *args: str, env: Mapping[str, str] | None = None) -> FoundryProcess:
        ...
        # - To make exec behave like other Foundry OS backends, we explicitly `cd` to our tracked
    ...
    async def unsafe_close(self) -> None:
        """Close the SSH connection. After that, SSHFoundryOS will be unusable."""
        if self._sftp:
            self._sftp.exit()
        if self._connection:
