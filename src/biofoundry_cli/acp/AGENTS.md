# ACP Integration Notes (biofoundry-cli)
...
- **Single-session server**: `BiofoundryCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/biofoundry_cli/app.py`, `src/biofoundry_cli/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/biofoundry_cli/acp/__init__.py`, `src/biofoundry_cli/acp/server.py`.
  - Exposed via the `biofoundry acp` command in `src/biofoundry_cli/cli/__init__.py`.
...
- `session/new`
  - Multi-session: creates a persisted `Session`, builds `BiofoundryCLI`, stores `ACPSession`.
...
- `session/load`
  - Multi-session only: loads by `Session.find`, then builds `BiofoundryCLI` and `ACPSession`.
...
- ACP sessions use `ACPFoundryOS` to route filesystem reads/writes through ACP clients.
...
- `session/set_mode` and `session/set_model` (no multi-mode/model switching in biofoundry-cli).
...
- When the client advertises `fs.readTextFile` / `fs.writeTextFile`, `ACPFoundryOS` routes
  reads and writes through ACP `fs/*` methods.
- `ReadFile` uses `FoundryPath.read_lines`, which `ACPFoundryOS` implements via ACP reads.
- `ReadMediaFile` uses `FoundryPath.read_bytes` to load image/video payloads through ACP reads.
- `WriteFile` uses `FoundryPath.read_text/write_text/append_text` and still generates diffs
