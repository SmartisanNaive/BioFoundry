...
from biofoundry_cli.acp.foundry_os import ACPFoundryOS
from biofoundry_cli.acp.mcp import acp_mcp_servers_to_mcp_config
from biofoundry_cli.acp.session import ACPSession
from biofoundry_cli.acp.tools import replace_tools
from biofoundry_cli.acp.types import ACPContentBlock, MCPServer
from biofoundry_cli.acp.version import ACPVersionSpec, negotiate_version
from biofoundry_cli.app import BiofoundryCLI
from biofoundry_cli.config import LLMModel, load_config, save_config
from biofoundry_cli.constant import NAME, VERSION
from biofoundry_cli.llm import create_llm, derive_model_capabilities
...
class ACPServer:
    ...
    async def new_session(
        ...
        session = await Session.create(FoundryPath.unsafe_from_local_path(Path(cwd)))

        mcp_config = acp_mcp_servers_to_mcp_config(mcp_servers or [])
        cli_instance = await BiofoundryCLI.create(
            session,
            mcp_configs=[mcp_config],
        )
        config = cli_instance.soul.runtime.config
        acp_foundry_os = ACPFoundryOS(self.conn, session.id, self.client_capabilities)
        acp_session = ACPSession(session.id, cli_instance, self.conn, foundry_os=acp_foundry_os)
        ...
        if isinstance(cli_instance.soul.agent.toolset, BiofoundryToolset):
            replace_tools(
    ...
    async def _setup_session(
        ...
        work_dir = FoundryPath.unsafe_from_local_path(Path(cwd))
    ...
    async def set_session_model(self, model_id: str, session_id: str, **kwargs: Any) -> None:
        ...
        assert config.is_from_default_location, (
            "`biofoundry acp` must use the default config location"
        )
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> acp.AuthenticateResponse | None:
        logger.warning(
            "ACP authenticate called but no auth methods are configured: {id}",
            id=method_id,
        )
        raise acp.RequestError.invalid_params({"method_id": "ACP authentication is not supported"})
