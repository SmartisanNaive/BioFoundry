...
from biofoundry_os import FoundryOS, reset_current_foundry_os, set_current_foundry_os

from biofoundry_cli.acp.convert import (
    acp_blocks_to_content_parts,
    display_block_to_acp_content,
    tool_result_to_acp_content,
)
from biofoundry_cli.acp.types import ACPContentBlock
from biofoundry_cli.app import BiofoundryCLI
from biofoundry_cli.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled
...
def get_current_acp_tool_call_id_or_none() -> str | None:
    ...
    from biofoundry_cli.soul.toolset import get_current_tool_call_or_none
...
class ACPSession:
    def __init__(
        ...
        cli: BiofoundryCLI,
        acp_conn: acp.Client,
        foundry_os: FoundryOS | None = None,
    ) -> None:
        self._id = id
        self._cli = cli
        self._conn = acp_conn
        self._foundry_os = foundry_os
    ...
    @property
    def cli(self) -> BiofoundryCLI:
        """The Biofoundry_CLI instance bound to this ACP session."""
        return self._cli
    ...
    async def prompt(self, prompt: list[ACPContentBlock]) -> acp.PromptResponse:
        ...
        foundry_os_token = (
            set_current_foundry_os(self._foundry_os) if self._foundry_os is not None else None
        )
        ...
        try:
            ...
            raise acp.RequestError.invalid_request(
                {
                    "error": (
                        ...
                        "OPENAI_MODEL_NAME before starting ACP."
                    )
                }
            ) from e
            ...
            if foundry_os_token is not None:
                reset_current_foundry_os(foundry_os_token)
