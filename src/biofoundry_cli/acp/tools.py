...
import acp
from biofoundry_llm.tooling import CallableTool2, ToolReturnValue
...
from biofoundry_cli.soul.approval import Approval
from biofoundry_cli.soul.toolset import BiofoundryToolset
from biofoundry_cli.tools.shell import Params as ShellParams
from biofoundry_cli.tools.shell import Shell
from biofoundry_cli.tools.utils import ToolRejectedError, ToolResultBuilder
from biofoundry_cli.wire.types import DisplayBlock
...
def replace_tools(
    ...
    current_foundry_os = get_current_foundry_os().name
    if current_foundry_os not in (local_foundry_os.name, "acp"):
        # Only replace tools when running locally or under ACPFoundryOS.
        return
...
class Terminal(CallableTool2[ShellParams]):
    ...
    async def __call__(self, params: ShellParams) -> ToolReturnValue:
        from biofoundry_cli.acp.session import get_current_acp_tool_call_id_or_none
