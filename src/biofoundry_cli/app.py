...
from biofoundry_cli.agentspec import DEFAULT_AGENT_FILE
from biofoundry_cli.cli import InputFormat, OutputFormat
from biofoundry_cli.config import Config, LLMModel, LLMProvider, load_config
from biofoundry_cli.llm import (
...
from biofoundry_cli.soul import run_soul
from biofoundry_cli.soul.agent import Runtime, load_agent
from biofoundry_cli.soul.biofoundrysoul import BiofoundrySoul
from biofoundry_cli.soul.context import Context
from biofoundry_cli.utils.aioqueue import QueueShutDown
from biofoundry_cli.utils.logging import logger, redirect_stderr_to_logger
from biofoundry_cli.utils.path import shorten_home
from biofoundry_cli.wire import Wire, WireUISide
from biofoundry_cli.wire.types import ContentPart, WireMessage
...
def enable_logging(debug: bool = False, *, redirect_stderr: bool = True) -> None:
    ...
    logger.enable("biofoundry_cli")
    if debug:
        logger.enable("biofoundry_llm")
    logger.add(
        get_share_dir() / "logs" / "biofoundry.log",
...
class BiofoundryCLI:
    ...
    async def create(
        ...
        *,
        ...
        """
        Create a BiofoundryCLI instance.
        ...
        Args:
            ...
            skills_dir (FoundryPath | None, optional): Override skills directory discovery. Defaults
                to None.
        ...
        Raises:
            ...
            ConfigError(BiofoundryCLIException, ValueError): When the configuration is invalid.
            AgentSpecError(BiofoundryCLIException, ValueError): When the agent specification
                is invalid.
            SystemPromptTemplateError(BiofoundryCLIException, ValueError): When the system prompt
                template is invalid.
            InvalidToolError(BiofoundryCLIException, ValueError): When any tool cannot be loaded.
            MCPConfigError(BiofoundryCLIException, ValueError): When any MCP configuration
                is invalid.
            MCPRuntimeError(BiofoundryCLIException, RuntimeError): When any MCP server cannot be
                connected.
        """
        ...
        soul = BiofoundrySoul(agent, context=context)
        return BiofoundryCLI(soul, runtime, env_overrides)

    def __init__(
        self,
        _soul: BiofoundrySoul,
        _runtime: Runtime,
    ...
    @property
    def soul(self) -> BiofoundrySoul:
        """Get the BiofoundrySoul instance."""
        return self._soul
    ...
    async def _env(self) -> AsyncGenerator[None]:
        original_cwd = FoundryPath.cwd()
        await biofoundry_os.chdir(self._runtime.session.work_dir)
        try:
            # to ignore possible warnings from dateparser
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            yield
        finally:
            await biofoundry_os.chdir(original_cwd)
    ...
    async def run(
        ...
        """
        Run the Biofoundry_CLI instance without any UI and yield Wire messages directly.
        ...
        Yields:
            WireMessage: The Wire messages from the `BiofoundrySoul`.
    ...
    async def run_shell(self, command: str | None = None) -> bool:
        """Run the Biofoundry_CLI instance with shell UI."""
        from biofoundry_cli.ui.shell import Shell, WelcomeInfoItem
        ...
        if self._env_overrides.get("OPENAI_API_KEY"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API Key",
                    value="****** (from OPENAI_API_KEY)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        if not self._runtime.llm:
            welcome_info.append(
                WelcomeInfoItem(
                    name="Model",
                    value="not configured, set OPENAI_API_KEY and optionally OPENAI_MODEL_NAME",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
            ...
            welcome_info.append(
                WelcomeInfoItem(
                    name="Tip",
                    value="use /model to switch between openai_responses and openai_legacy",
                    level=WelcomeInfoItem.Level.INFO,
                )
            )
        async with self._env():
    ...
    async def run_print(
        ...
        *,
        ...
        """Run the Biofoundry_CLI instance with print UI."""
        from biofoundry_cli.ui.print import Print
    ...
    async def run_acp(self) -> None:
        """Run the Biofoundry_CLI instance as ACP server."""
        from biofoundry_cli.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the Biofoundry_CLI instance as Wire server over stdio."""
        from biofoundry_cli.wire.server import WireServer
