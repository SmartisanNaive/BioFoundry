...
cli = typer.Typer(
    ...
    help="Biofoundry_CLI, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"biofoundry, version {VERSION}")
        raise typer.Exit()
...
def biofoundry(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ./.biofoundry/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `biofoundry acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    """Biofoundry_CLI, your next CLI agent."""
    from biofoundry_cli.utils.proctitle import init_process_name

    init_process_name("Biofoundry_CLI")
    ...
    from biofoundry_cli.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from biofoundry_cli.app import BiofoundryCLI, enable_logging
    from biofoundry_cli.config import Config, load_config_from_string
    ...
    # We re-enable stderr redirection after BiofoundryCLI.create() succeeds.
    ...
    if local_skills_dir is not None:
        skills_dir = FoundryPath.unsafe_from_local_path(local_skills_dir)

    work_dir = (
        FoundryPath.unsafe_from_local_path(local_work_dir) if local_work_dir else FoundryPath.cwd()
    )
    ...
    async def _run(session_id: str | None) -> tuple[Session, bool]:
        ...
        if local_add_dirs:
            from biofoundry_cli.utils.path import is_within_directory

            canonical_work_dir = work_dir.canonical()
            changed = False
            for d in local_add_dirs:
                dir_path = FoundryPath.unsafe_from_local_path(d).canonical()
        ...
        instance = await BiofoundryCLI.create(
            session,
            config=config,
    ...
    try:
        ...
        if debug:
            ...
            log_path = get_share_dir() / "logs" / "biofoundry.log"
...
def term(
    ...
    """Run Toad TUI backed by Biofoundry_CLI ACP server."""
...
if __name__ == "__main__":
    if "biofoundry_cli.cli" not in sys.modules:
        sys.modules["biofoundry_cli.cli"] = sys.modules[__name__]
