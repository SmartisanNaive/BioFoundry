...
def _collect_info() -> InfoData:
    from biofoundry_cli.agentspec import SUPPORTED_AGENT_SPEC_VERSIONS
    from biofoundry_cli.wire.protocol import WIRE_PROTOCOL_VERSION

    return {
        "biofoundry_cli_version": VERSION,
...
def _emit_info(json_output: bool) -> None:
    ...
    lines = [
        f"biofoundry-cli version: {info['biofoundry_cli_version']}",
