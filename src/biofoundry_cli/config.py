...
class Config(BaseModel):
    ...
    def validate_model(self) -> Self:
        ...
        if not self.default_model:
            raise ValueError("default_model must be set")
...
def get_default_config() -> Config:
    ...
    return Config(
        ...
        models={
            "openai-responses": LLMModel(
                provider="openai-responses",
                model=DEFAULT_OPENAI_MODEL,
                max_context_size=100_000,
            ),
            "openai-legacy": LLMModel(
        ...
        providers={
            "openai-responses": LLMProvider(
                type="openai_responses",
                base_url=DEFAULT_OPENAI_BASE_URL,
                api_key=SecretStr(""),
            ),
            "openai-legacy": LLMProvider(
                type="openai_legacy",
                base_url=DEFAULT_OPENAI_BASE_URL,
                api_key=SecretStr(""),
            ),
...
def load_config(config_file: Path | None = None) -> Config:
    ...
    The config file must exist before startup.
    ...
    if not config_file.exists():
        if is_default_config_file:
            raise ConfigError(
                ...
                f"{config_file}. Create .biofoundry/config.toml in the project root first. "
                "Use the project-root config.example.toml as a template."
            )
        raise ConfigError(
            f"Config file not found: {config_file}. Create it first before running Biofoundry_CLI."
        )
