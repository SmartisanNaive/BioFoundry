...
from biofoundry_llm.chat_provider import ChatProvider
from pydantic import SecretStr

if TYPE_CHECKING:
    from biofoundry_cli.config import LLMModel, LLMProvider
...
def create_llm(
    ...
    *,
    ...
    match provider.type:
        case "openai_legacy":
            from biofoundry_llm.contrib.chat_provider.openai_legacy import OpenAILegacy
        ...
        case "anthropic":
            from biofoundry_llm.contrib.chat_provider.anthropic import Anthropic
        ...
        case "google_genai" | "gemini":
            from biofoundry_llm.contrib.chat_provider.google_genai import GoogleGenAI
        ...
        case "_echo":
            from biofoundry_llm.chat_provider.echo import EchoChatProvider

            chat_provider = EchoChatProvider()
        case "_scripted_echo":
            from biofoundry_llm.chat_provider.echo import ScriptedEchoChatProvider

            if provider.env:
                os.environ.update(provider.env)
            scripts = _load_scripted_echo_scripts()
            trace_value = os.getenv("BIOFOUNDRY_SCRIPTED_ECHO_TRACE", "")
            trace = trace_value.strip().lower() in {"1", "true", "yes", "on"}
            chat_provider = ScriptedEchoChatProvider(scripts, trace=trace)
        case "_chaos":
            from biofoundry_llm.chat_provider.chaos import ChaosChatProvider, ChaosConfig
            from biofoundry_llm.contrib.chat_provider.openai_responses import OpenAIResponses
...
def _load_scripted_echo_scripts() -> list[str]:
    script_path = os.getenv("BIOFOUNDRY_SCRIPTED_ECHO_SCRIPTS")
    if not script_path:
        raise ValueError("BIOFOUNDRY_SCRIPTED_ECHO_SCRIPTS is required for _scripted_echo.")
