...
from biofoundry_cli.resources import get_builtin_skills_dir_candidates as _get_builtin_skills_dirs
from biofoundry_cli.skill.flow import Flow, FlowError
from biofoundry_cli.skill.flow.d2 import parse_d2_flowchart
from biofoundry_cli.skill.flow.mermaid import parse_mermaid_flowchart
from biofoundry_cli.utils.frontmatter import parse_frontmatter
...
def get_builtin_skills_dir() -> Path:
    ...
    """
    candidates = _get_builtin_skills_dirs()
    if candidates:
        return candidates[0]
...
def get_builtin_skills_dir_candidates() -> tuple[Path, ...]:
    """
    Get built-in skill directory candidates that are independent from the current cwd.
    """
    return _get_builtin_skills_dirs()


def get_user_skills_dir_candidates() -> tuple[FoundryPath, ...]:
    """
    Get user-level skills directory candidates in priority order.
    """
    return (
        FoundryPath.home() / ".config" / "agents" / "skills",
        FoundryPath.home() / ".agents" / "skills",
        FoundryPath.home() / ".biofoundry" / "skills",
        FoundryPath.home() / ".claude" / "skills",
        FoundryPath.home() / ".codex" / "skills",
    )


def get_project_skills_dir_candidates(work_dir: FoundryPath) -> tuple[FoundryPath, ...]:
    """
    Get project-level skills directory candidates in priority order.
    """
    return (
        work_dir / ".agents" / "skills",
        work_dir / ".biofoundry" / "skills",
...
def _supports_builtin_skills() -> bool:
    """Return True when the active Foundry OS backend can read bundled skills."""
    current_name = get_current_foundry_os().name
    return current_name in (local_foundry_os.name, "acp")


async def find_first_existing_dir(candidates: Iterable[FoundryPath]) -> FoundryPath | None:
    """
...
async def find_user_skills_dir() -> FoundryPath | None:
    """
...
async def find_project_skills_dir(work_dir: FoundryPath) -> FoundryPath | None:
    """
...
async def resolve_skills_roots(
    work_dir: FoundryPath,
    *,
    skills_dir_override: FoundryPath | None = None,
) -> list[FoundryPath]:
    """
    Resolve layered skill roots in priority order.

    Built-in skills load first when supported by the active Foundry OS backend. When an
    override is provided, user/project discovery is skipped.
    """
    roots: list[FoundryPath] = []
    if _supports_builtin_skills():
        roots.extend(
            FoundryPath.unsafe_from_local_path(path) for path in get_builtin_skills_dir_candidates()
        )
...
async def discover_skills_from_roots(skills_dirs: Iterable[FoundryPath]) -> list[Skill]:
    """
...
class Skill(BaseModel):
    ...
    @property
    def skill_md_file(self) -> FoundryPath:
        """Path to the SKILL.md file."""
        return self.dir / "SKILL.md"


async def discover_skills(skills_dir: FoundryPath) -> list[Skill]:
    """
    Discover all skills in the given directory.

    Args:
        skills_dir: FoundryOS path to the directory containing skills.
...
def parse_skill_text(content: str, *, dir_path: FoundryPath) -> Skill:
    """
