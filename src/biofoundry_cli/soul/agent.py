...
import pydantic
from biofoundry_llm.tooling import Toolset
from biofoundry_os.path import FoundryPath
from jinja2 import Environment as JinjaEnvironment
from jinja2 import StrictUndefined, TemplateError, UndefinedError

from biofoundry_cli.agentspec import load_agent_spec
from biofoundry_cli.config import Config
from biofoundry_cli.exception import MCPConfigError, SystemPromptTemplateError
from biofoundry_cli.llm import LLM
...
from biofoundry_cli.skill import (
    Skill,
    discover_skills_from_roots,
    index_skills,
    resolve_skills_roots,
)
from biofoundry_cli.soul.approval import Approval, ApprovalState
from biofoundry_cli.soul.denwarenji import DenwaRenji
from biofoundry_cli.soul.toolset import BiofoundryToolset
from biofoundry_cli.utils.environment import Environment
from biofoundry_cli.utils.logging import logger
from biofoundry_cli.utils.path import list_directory
...
class BuiltinSystemPromptArgs:
    ...
    BIOFOUNDRY_WORK_DIR: FoundryPath
    """The absolute path of current working directory."""
    BIOFOUNDRY_WORK_DIR_LS: str
    """The directory listing of current working directory."""
    BIOFOUNDRY_AGENTS_MD: str  # TODO: move to first message from system prompt
    """The content of AGENTS.md."""
    BIOFOUNDRY_SKILLS: str
    """Formatted information about available skills."""
    BIOFOUNDRY_KNOWLEDGE: str
    """Formatted information about bundled knowledge resources."""
    BIOFOUNDRY_ADDITIONAL_DIRS_INFO: str
    """Formatted information about additional directories in the workspace."""


def _dedupe_foundry_paths(paths: list[FoundryPath]) -> list[FoundryPath]:
    deduped: list[FoundryPath] = []
    seen: set[str] = set()
    for path in paths:
...
async def load_agents_md(work_dir: FoundryPath) -> str | None:
    paths = [
        work_dir / "AGENTS.md",
        work_dir / "agents.md",
    ]
...
class Runtime:
    ...
    additional_dirs: list[FoundryPath]
    ...
    async def create(
        ...
        knowledge_formatted = format_builtin_knowledge_index()

        embedded_workspace_dirs = [
            FoundryPath.unsafe_from_local_path(path)
            for path in (
                *get_builtin_skills_dir_candidates(),
                *get_builtin_knowledge_dir_candidates(),
            )
        ...
        user_additional_dirs: list[FoundryPath] = []
        pruned = False
        valid_dir_strs: list[str] = []
        for dir_str in session.state.additional_dirs:
            d = FoundryPath(dir_str).canonical()
            if await d.is_dir():
                user_additional_dirs.append(d)
        ...
        if pruned:
            ...
            session.save_state()
        additional_dirs = _dedupe_foundry_paths(
            [
                *user_additional_dirs,
                *embedded_workspace_dirs,
            ]
        )
        ...
        if user_additional_dirs:
            parts: list[str] = []
            for d in user_additional_dirs:
                try:
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                BIOFOUNDRY_NOW=datetime.now().astimezone().isoformat(),
                BIOFOUNDRY_WORK_DIR=session.work_dir,
                BIOFOUNDRY_WORK_DIR_LS=ls_output,
                BIOFOUNDRY_AGENTS_MD=agents_md or "",
                BIOFOUNDRY_SKILLS=skills_formatted or "No skills found.",
                BIOFOUNDRY_KNOWLEDGE=knowledge_formatted,
                BIOFOUNDRY_ADDITIONAL_DIRS_INFO=additional_dirs_info,
            ),
            denwa_renji=DenwaRenji(),
...
async def load_agent(
    ...
    *,
    ...
    Raises:
        ...
        AgentSpecError(BiofoundryCLIException, ValueError): When the agent specification is invalid.
        SystemPromptTemplateError(BiofoundryCLIException, ValueError): When the
            system prompt template is invalid.
        InvalidToolError(BiofoundryCLIException, ValueError): When any tool cannot be loaded.
        MCPConfigError(BiofoundryCLIException, ValueError): When any MCP configuration is invalid.
        MCPRuntimeError(BiofoundryCLIException, RuntimeError): When any MCP server
            cannot be connected.
    """
    ...
    toolset = BiofoundryToolset()
    tool_deps = {
        BiofoundryToolset: toolset,
        Runtime: runtime,
