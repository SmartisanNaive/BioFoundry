...
from biofoundry_cli.llm import ModelCapability
from biofoundry_cli.skill import Skill, read_skill_text
from biofoundry_cli.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
...
from biofoundry_cli.soul.agent import Agent, Runtime
from biofoundry_cli.soul.compaction import CompactionResult, SimpleCompaction, should_auto_compact
from biofoundry_cli.soul.context import Context
from biofoundry_cli.soul.dynamic_injection import (
    DynamicInjection,
    DynamicInjectionProvider,
    normalize_history,
)
from biofoundry_cli.soul.dynamic_injections.plan_mode import PlanModeInjectionProvider
from biofoundry_cli.soul.message import (
    check_message,
    system,
    system_reminder,
    tool_result_to_message,
)
from biofoundry_cli.soul.slash import registry as soul_slash_registry
from biofoundry_cli.soul.toolset import BiofoundryToolset
from biofoundry_cli.tools.dmail import NAME as SendDMail_NAME
from biofoundry_cli.tools.utils import ToolRejectedError
from biofoundry_cli.utils.logging import logger
from biofoundry_cli.utils.slashcmd import SlashCommand, parse_slash_command_call
from biofoundry_cli.wire.file import WireFile
from biofoundry_cli.wire.types import (
    ApprovalRequest,
    ApprovalResponse,
    CompactionBegin,
    CompactionEnd,
...
if TYPE_CHECKING:
    ...
    def type_check(soul: BiofoundrySoul):
        _: Soul = soul
...
class BiofoundrySoul:
    """The soul of Biofoundry_CLI."""
    ...
    def _bind_plan_mode_tools(self) -> None:
        """Bind plan mode state to tools that support it."""
        if not isinstance(self._agent.toolset, BiofoundryToolset):
            return
        ...
        from biofoundry_cli.tools.file.write import WriteFile
        ...
        from biofoundry_cli.tools.plan import ExitPlanMode
        ...
        from biofoundry_cli.tools.plan.enter import EnterPlanMode
        ...
        from biofoundry_cli.tools.ask_user import AskUserQuestion
    ...
    def get_plan_file_path(self) -> Path | None:
        ...
        if self._plan_session_id is None:
            return None
        from biofoundry_cli.tools.plan.heroes import get_plan_file_path
    ...
    def read_current_plan(self) -> str | None:
        ...
        if self._plan_session_id is None:
            return None
        from biofoundry_cli.tools.plan.heroes import read_plan_file
    ...
    def _make_skill_runner(
        self, skill: Skill
    ) -> Callable[[BiofoundrySoul, str], None | Awaitable[None]]:
        async def _run_skill(soul: BiofoundrySoul, args: str, *, _skill: Skill = skill) -> None:
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, BiofoundryToolset):
    ...
    async def _step(self) -> StepOutcome | None:
        ...
        async def _run_step_once() -> StepResult:
            ...
            return await biofoundry_llm.step(
                chat_provider,
        ...
        async def _llm_step_with_retry() -> StepResult:
        ...
        result = await _llm_step_with_retry()
    ...
    def _is_retryable_error(exception: BaseException) -> bool:
        if isinstance(exception, (APIConnectionError, APITimeoutError)):
            return not bool(getattr(exception, "_biofoundry_recovery_exhausted", False))
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                second_error._biofoundry_recovery_exhausted = True  # type: ignore[attr-defined]
                raise
...
class FlowRunner:
    ...
    async def run(self, soul: BiofoundrySoul, args: str) -> None:
        if args.strip():
