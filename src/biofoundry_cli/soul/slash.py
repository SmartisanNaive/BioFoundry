...
import biofoundry_cli.prompts as prompts
from biofoundry_cli.soul import wire_send
from biofoundry_cli.soul.agent import load_agents_md
from biofoundry_cli.soul.context import Context
from biofoundry_cli.soul.message import system
from biofoundry_cli.utils.export import is_sensitive_file
from biofoundry_cli.utils.path import sanitize_cli_path, shorten_home
from biofoundry_cli.utils.slashcmd import SlashCommandRegistry
from biofoundry_cli.wire.types import StatusUpdate, TextPart

if TYPE_CHECKING:
    from biofoundry_cli.soul.biofoundrysoul import BiofoundrySoul

type SoulSlashCmdFunc = Callable[[BiofoundrySoul, str], None | Awaitable[None]]
"""
A function that runs as a BiofoundrySoul-level slash command.
...
@registry.command
async def init(soul: BiofoundrySoul, args: str):
    """Analyze the codebase and generate an `AGENTS.md` file"""
    from biofoundry_cli.soul.biofoundrysoul import BiofoundrySoul

    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_context = Context(file_backend=Path(temp_dir) / "context.jsonl")
        tmp_soul = BiofoundrySoul(soul.agent, context=tmp_context)
        await tmp_soul.run(prompts.INIT)

    agents_md = await load_agents_md(soul.runtime.builtin_args.BIOFOUNDRY_WORK_DIR)
...
@registry.command
async def compact(soul: BiofoundrySoul, args: str):
...
async def clear(soul: BiofoundrySoul, args: str):
    """Clear the context"""
...
@registry.command
async def yolo(soul: BiofoundrySoul, args: str):
...
@registry.command
async def plan(soul: BiofoundrySoul, args: str):
...
async def add_dir(soul: BiofoundrySoul, args: str):
    """Add a directory to the workspace. Usage: /add-dir <path>. Run without args to list added dirs"""  # noqa: E501
    from biofoundry_os.path import FoundryPath

    from biofoundry_cli.utils.path import is_within_directory, list_directory
    ...
    path = FoundryPath(args).expanduser().canonical()
    ...
    work_dir = soul.runtime.builtin_args.BIOFOUNDRY_WORK_DIR
...
@registry.command
async def export(soul: BiofoundrySoul, args: str):
    """Export current session context to a markdown file"""
    from biofoundry_cli.utils.export import perform_export
    ...
    if isinstance(result, str):
        ...
        return
    output, count = result
    display = shorten_home(FoundryPath(str(output)))
...
async def import_context(soul: BiofoundrySoul, args: str):
    """Import context from a file or session ID"""
    from biofoundry_cli.utils.export import perform_import
