...
from biofoundry_llm.tooling import (
    CallableTool,
    CallableTool2,
    HandleResult,
    Tool,
...
from biofoundry_llm.tooling.error import (
    ToolNotFoundError,
    ToolParseError,
    ToolRuntimeError,
)
from biofoundry_llm.tooling.mcp import convert_mcp_content
from biofoundry_llm.utils.typing import JsonType
from loguru import logger

from biofoundry_cli.exception import InvalidToolError, MCPRuntimeError
from biofoundry_cli.tools import SkipThisTool
...
if TYPE_CHECKING:
    ...
    def type_check(biofoundry_toolset: BiofoundryToolset):
        _: Toolset = biofoundry_toolset


class BiofoundryToolset:
    ...
    def load_tools(self, tool_paths: list[str], dependencies: dict[type[Any], Any]) -> None:
        """
        Load tools from paths like `biofoundry_cli.tools.shell:Shell`.
    ...
    async def load_mcp_tools(
        ...
        from biofoundry_cli.ui.shell.prompt import toast
            ...
            for server_name, server_info in self._mcp_servers.items():
                ...
                if not await _check_oauth_tokens(server_url):
                    logger.warning(
                        ...
                        "Run 'biofoundry mcp auth {server_name}' first.",
                        server_name=server_name,
                    )
...
class WireExternalTool(CallableTool):
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> ToolReturnValue:
        ...
        from biofoundry_cli.soul import get_wire_or_none
...
def convert_mcp_tool_result(result: CallToolResult) -> ToolReturnValue:
    """Convert MCP tool result to a biofoundry_llm tool return value.
