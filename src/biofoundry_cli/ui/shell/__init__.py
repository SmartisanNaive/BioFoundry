...
from biofoundry_llm.chat_provider import APIStatusError, ChatProviderError
...
from biofoundry_cli.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    RunCancelled,
...
from biofoundry_cli.ui.shell import update as _update_mod
from biofoundry_cli.ui.shell.console import console
from biofoundry_cli.ui.shell.echo import render_user_echo_text
from biofoundry_cli.ui.shell.prompt import (
    CustomPromptSession,
    PromptMode,
    UserInput,
    toast,
)
from biofoundry_cli.ui.shell.replay import replay_recent_history
from biofoundry_cli.ui.shell.slash import registry as shell_slash_registry
from biofoundry_cli.ui.shell.slash import shell_mode_registry
from biofoundry_cli.ui.shell.update import (
    LATEST_VERSION_FILE,
    UpdateResult,
    do_update,
    semver_tuple,
)
from biofoundry_cli.ui.shell.visualize import visualize
from biofoundry_cli.utils.envvar import get_env_bool
from biofoundry_cli.utils.logging import open_original_stderr
from biofoundry_cli.utils.signals import install_sigint_handler
from biofoundry_cli.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from biofoundry_cli.utils.subprocess_env import get_clean_env
from biofoundry_cli.utils.term import ensure_new_line, ensure_tty_sane
from biofoundry_cli.wire.types import ContentPart, StatusUpdate
...
class Shell:
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if get_env_bool("BIOFOUNDRY_CLI_NO_AUTO_UPDATE"):
            logger.info(
                "Auto-update disabled by BIOFOUNDRY_CLI_NO_AUTO_UPDATE environment variable"
            )
        else:
            self._start_background_task(self._auto_update())

        _print_welcome_info(self.soul.name or "Biofoundry_CLI", self._welcome_info)

        if isinstance(self.soul, BiofoundrySoul):
    ...
    async def _run_slash_command(self, command_call: SlashCommandCall) -> None:
        from biofoundry_cli.cli import Reload, SwitchToWeb
        ...
        try:
            ...
            await run_soul(
                ...
                lambda wire: visualize(
                    ...
                    steer=self.soul.steer if isinstance(self.soul, BiofoundrySoul) else None,
                ),
                cancel_event,
                self.soul.wire_file if isinstance(self.soul, BiofoundrySoul) else None,
            )
            return True
        except LLMNotSet:
            logger.exception("LLM not set:")
            console.print(
                "[red]LLM not set. Configure OPENAI_API_KEY and optionally OPENAI_MODEL_NAME.[/red]"
            )
            ...
            if isinstance(e, APIStatusError) and e.status_code == 401:
                console.print("[red]Authorization failed, please check your OPENAI_API_KEY[/red]")
            elif isinstance(e, APIStatusError) and e.status_code == 402:
                console.print(
                    "[red]Upstream provider rejected the request for billing reasons[/red]"
                )
            elif isinstance(e, APIStatusError) and e.status_code == 403:
                console.print(
                    "[red]Request forbidden or quota exceeded on the upstream provider[/red]"
                )
            else:
...
_WELCOME_ACCENT = "#57e3d0"
_WELCOME_SUPPORT = "#c88cff"
_WELCOME_HIGHLIGHT = "#a8f56d"
_WELCOME_MUTED = "grey70"
_LOGO = r""" ________  ___  ________  ________ ________  ___  ___  ________   ________  ________      ___    ___
|\   __  \|\  \|\   __  \|\  _____\\   __  \|\  \|\  \|\   ___  \|\   ___ \|\   __  \    |\  \  /  /|
\ \  \|\ /\ \  \ \  \|\  \ \  \__/\ \  \|\  \ \  \\\  \ \  \\ \  \ \  \_|\ \ \  \|\  \   \ \  \/  / /
 \ \   __  \ \  \ \  \\\  \ \   __\\ \  \\\  \ \  \\\  \ \  \\ \  \ \  \ \\ \ \   _  _\   \ \    / /
  \ \  \|\  \ \  \ \  \\\  \ \  \_| \ \  \\\  \ \  \\\  \ \  \\ \  \ \  \_\\ \ \  \\  \|   \/  /  /
   \ \_______\ \__\ \_______\ \__\   \ \_______\ \_______\ \__\\ \__\ \_______\ \__\\ _\ __/  / /
    \|_______|\|__|\|_______|\|__|    \|_______|\|_______|\|__| \|__|\|_______|\|__|\|__|\___/ /
                                                                                        \|___|/
"""
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    rows: list[RenderableType] = [
        Text(_LOGO, style=_WELCOME_ACCENT, no_wrap=True, overflow="ignore")
    ]
    ...
    if LATEST_VERSION_FILE.exists():
        from biofoundry_cli.constant import VERSION as current_version
    ...
    console.print(
        Panel(
            Group(*rows),
            border_style=_WELCOME_ACCENT,
            box=box.ROUNDED,
            expand=False,
            padding=(1, 2),
            title=f"[bold {_WELCOME_ACCENT}]{name}[/]",
            subtitle=f"[{_WELCOME_MUTED}]sequencing session[/]",
        )
    )
