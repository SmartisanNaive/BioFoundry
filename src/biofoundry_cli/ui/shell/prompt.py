...
from biofoundry_cli.soul import StatusSnapshot, format_context_status
from biofoundry_cli.ui.shell import placeholders as prompt_placeholders
from biofoundry_cli.ui.shell.console import console
from biofoundry_cli.ui.shell.placeholders import (
    PromptPlaceholderManager,
    normalize_pasted_text,
    sanitize_surrogates,
)
from biofoundry_cli.utils.clipboard import (
    grab_media_from_clipboard,
    is_clipboard_available,
)
from biofoundry_cli.utils.logging import logger
from biofoundry_cli.utils.slashcmd import SlashCommand
from biofoundry_cli.wire.types import ContentPart
...
PROMPT_THEME_ACCENT = "#57e3d0"
PROMPT_THEME_SUPPORT = "#c88cff"
PROMPT_THEME_HIGHLIGHT = "#a8f56d"
PROMPT_THEME_TEXT = "#e8fff8"
PROMPT_THEME_MUTED = "#6a8f95"
PROMPT_THEME_BG = "#07161a"
PROMPT_THEME_BG_ALT = "#0c2228"
...
class CustomPromptSession:
    def __init__(
        ...
        *,
        ...
        work_dir_id = md5(str(FoundryPath.cwd()).encode(encoding="utf-8")).hexdigest()
        ...
        self._agent_mode_completer = merge_completers(
            [
                ...
                # TODO(foundry_os): we need an async FoundryOSFileMentionCompleter
                LocalFileMentionCompleter(FoundryPath.cwd().unsafe_to_local_path()),
            ],
            deduplicate=True,
        )
        ...
        self._session = PromptSession[str](
            ...
            style=Style.from_dict(
                {
                    "bottom-toolbar": (
                        f"fg:{PROMPT_THEME_TEXT} bg:{PROMPT_THEME_BG} noreverse"
                    ),
                    "running-prompt-placeholder": f"fg:{PROMPT_THEME_MUTED} italic",
                    "running-prompt-separator": f"fg:{PROMPT_THEME_MUTED}",
                    "slash-completion-menu": f"bg:{PROMPT_THEME_BG_ALT}",
                    "slash-completion-menu.separator": (
                        f"fg:{PROMPT_THEME_MUTED} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                    "slash-completion-menu.marker": (
                        f"fg:{PROMPT_THEME_MUTED} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                    "slash-completion-menu.marker.current": (
                        f"fg:{PROMPT_THEME_HIGHLIGHT} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                    "slash-completion-menu.command": (
                        f"fg:{PROMPT_THEME_TEXT} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                    "slash-completion-menu.meta": (
                        f"fg:{PROMPT_THEME_MUTED} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                    "slash-completion-menu.command.current": (
                        f"fg:{PROMPT_THEME_ACCENT} bg:{PROMPT_THEME_BG_ALT} bold"
                    ),
                    "slash-completion-menu.meta.current": (
                        f"fg:{PROMPT_THEME_SUPPORT} bg:{PROMPT_THEME_BG_ALT}"
                    ),
                }
            ),
        )
    ...
    def _render_message(self) -> FormattedText:
        if self._mode == PromptMode.SHELL:
            return FormattedText([(f"fg:{PROMPT_THEME_ACCENT} bold", f"{PROMPT_SYMBOL_SHELL} ")])
    ...
    def _open_in_external_editor(self, event: KeyPressEvent) -> None:
        ...
        from biofoundry_cli.utils.editor import edit_text_in_editor, get_editor_command
    ...
    def _render_agent_prompt_label(self) -> FormattedText:
        ...
        if status.plan_mode:
            return FormattedText([(f"fg:{PROMPT_THEME_SUPPORT}", f"{PROMPT_SYMBOL_PLAN} ")])
        symbol = PROMPT_SYMBOL_THINKING if self._thinking else PROMPT_SYMBOL
        color = PROMPT_THEME_SUPPORT if self._thinking else PROMPT_THEME_ACCENT
        return FormattedText([(f"fg:{color}", f"{symbol} ")])
    ...
    def _render_bottom_toolbar(self) -> FormattedText:
        ...
        fragments.append((f"fg:{PROMPT_THEME_MUTED}", "─" * columns))
        fragments.append(("", "\n"))
        ...
        if status.yolo_enabled:
            fragments.extend([(f"bold fg:{PROMPT_THEME_HIGHLIGHT}", "yolo"), ("", " " * 2)])
            columns -= len("yolo") + 2
        if status.plan_mode:
            fragments.extend([(f"bold fg:{PROMPT_THEME_SUPPORT}", "plan"), ("", " " * 2)])
            columns -= len("plan") + 2
