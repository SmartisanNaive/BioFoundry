...
from biofoundry_cli.cli import Reload
from biofoundry_cli.config import load_config, save_config
...
from biofoundry_cli.utils.changelog import CHANGELOG
from biofoundry_cli.utils.datetime import format_relative_time
from biofoundry_cli.utils.slashcmd import SlashCommand, SlashCommandRegistry

if TYPE_CHECKING:
    from biofoundry_cli.ui.shell import Shell
...
def ensure_biofoundry_soul(app: Shell) -> BiofoundrySoul | None:
    if not isinstance(app.soul, BiofoundrySoul):
        console.print("[red]Interactive agent required[/red]")
        return None
    return app.soul
...
def help(app: Shell, args: str):
    ...
    from biofoundry_cli.utils.rich.columns import BulletColumns
    ...
    renderables.append(
        BulletColumns(
            Text(
                "Biofoundry_CLI is ready to help! "
...
def version(app: Shell, args: str):
    ...
    console.print(f"biofoundry, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from biofoundry_cli.llm import derive_model_capabilities

    soul = ensure_biofoundry_soul(app)
    if soul is None:
        return
    ...
    if not config.models:
        console.print(
            "[yellow]No models configured. Set OPENAI_API_KEY or add models to "
            "./.biofoundry/config.toml.[/yellow]"
        )
        return
    ...
    for name in sorted(config.models):
        ...
        provider_label = (
            provider_cfg.type.replace("_", " ") if provider_cfg is not None else model_cfg.provider
        )
...
async def editor(app: Shell, args: str):
    ...
    from biofoundry_cli.utils.editor import get_editor_command
...
def feedback(app: Shell, args: str):
    """Submit feedback to make Biofoundry_CLI better"""
    console.print("Feedback endpoint is not configured for this fork.")


@registry.command(aliases=["reset"])
async def clear(app: Shell, args: str):
    """Clear the context"""
    if ensure_biofoundry_soul(app) is None:
        return
...
async def mcp(app: Shell, args: str):
    ...
    if not isinstance(toolset, BiofoundryToolset):
        console.print("[red]BiofoundryToolset required[/red]")
        return
    ...
    for name, info in servers.items():
        ...
        if info.status == "unauthorized":
            server_text += " [grey50](unauthorized - run: biofoundry mcp auth {name})[/grey50]"
