...
class Environment:
    ...
    async def detect() -> Environment:
        ...
        if os_kind == "Windows":
            ...
            shell_path = FoundryPath("powershell.exe")
        else:
            possible_paths = [
                FoundryPath("/bin/bash"),
                FoundryPath("/usr/bin/bash"),
                FoundryPath("/usr/local/bin/bash"),
            ]
            fallback_path = FoundryPath("/bin/sh")
