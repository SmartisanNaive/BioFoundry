...
async def list_directory(work_dir: FoundryPath) -> str:
    """Return an ``ls``-like listing of *work_dir*.

    This helper is used mainly to provide context to the LLM (for example
    ``BIOFOUNDRY_WORK_DIR_LS``) and to show top-level directory contents in tools.
...
def shorten_home(path: FoundryPath) -> FoundryPath:
    """
...
def is_within_directory(path: FoundryPath, directory: FoundryPath) -> bool:
    """
    Check whether *path* is contained within *directory* using pure path semantics.
    Both arguments should already be canonicalized (e.g. via FoundryPath.canonical()).
    """
...
def is_within_workspace(
    ...
    additional_dirs: Sequence[FoundryPath] = (),
) -> bool:
    """
