...
import pydantic
from biofoundry_llm.chat_provider import ChatProviderError
from biofoundry_llm.tooling import ToolError, ToolResult
from biofoundry_llm.utils.typing import JsonType
...
from biofoundry_cli.soul.toolset import BiofoundryToolset, WireExternalTool
from biofoundry_cli.utils.aioqueue import Queue, QueueShutDown
from biofoundry_cli.utils.logging import logger
from biofoundry_cli.utils.signals import install_sigint_handler
from biofoundry_cli.wire import Wire
...
class WireServer:
    ...
    async def _handle_initialize(
        ...
        toolset = None
        if isinstance(self._soul, BiofoundrySoul) and isinstance(
            self._soul.agent.toolset, BiofoundryToolset
        ):
        ...
        from biofoundry_cli.constant import NAME, VERSION
        from biofoundry_cli.wire.protocol import WIRE_PROTOCOL_VERSION
    ...
    def _sync_ask_user_tool_visibility(self, toolset: BiofoundryToolset) -> None:
        """Hide or unhide the AskUserQuestion tool based on client capabilities."""
        from biofoundry_cli.tools.ask_user import NAME as ASK_USER_TOOL_NAME

        all_toolsets = [toolset]
        if isinstance(self._soul, BiofoundrySoul):
            for subagent in self._soul.agent.runtime.labor_market.fixed_subagents.values():
                if isinstance(subagent.toolset, BiofoundryToolset):
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: BiofoundryToolset) -> None:
        """Hide or unhide plan mode tools based on client capabilities."""
        from biofoundry_cli.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from biofoundry_cli.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if not isinstance(self._soul, BiofoundrySoul):
            return
        ...
        client_obj = getattr(llm.chat_provider, "client", None)
        custom_headers = getattr(client_obj, "_custom_headers", None)
    ...
    async def _handle_prompt(
        ...
        try:
            await run_soul(
                ...
                self._cancel_event,
                self._soul.wire_file if isinstance(self._soul, BiofoundrySoul) else None,
            )
            return JSONRPCSuccessResponse(
                id=msg.id,
    ...
    async def _handle_steer(
        ...
        if not isinstance(self._soul, BiofoundrySoul) or not self._is_streaming:
            return JSONRPCErrorResponse(
                id=msg.id,
                error=JSONRPCErrorObject(
                    code=ErrorCodes.INVALID_STATE,
    ...
    async def _handle_replay(
        ...
        wire_file = self._soul.wire_file if isinstance(self._soul, BiofoundrySoul) else None
